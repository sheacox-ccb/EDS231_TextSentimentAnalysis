---
title: "Topic5_WordRelationships"
author: "Julia Parish"
date: '2022-04-27'
...
```{r co-occur_plots}
word_pairs %>%
  filter(n >= 70) %>% # filtering for cooccurances
...
# interested in how the EPA is discussing enviro justice in the context of equity and income
  word_cors %>%
...
```{r corr_network}
# focus on particular corr network, term of interest is at the center, what are the other words that correlate most highly with "justice"
justice_cors  %>%
  filter(n <= 50) %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
...
#first the basic frequency stat - each year, most frequent words 
...
```{r convert_dfm}
# n-gram: multi-word, 2 word pattern as fundamental unit of analysis
toks2 <- tokens_ngrams(toks1, n=2)

dfm2 <- dfm(toks2)
dfm2 <- dfm_remove(dfm2, pattern = c(stop_vec))

# calculate frequency of 2 word pairings
...
```{r}
# reference is words from non-2015 docs, target = 1 is list of documents so selecting 2015 doc
