...
```{r, eval=FALSE}
data <- "/Users/julia/Documents/_MEDS/04_spring/EDS231_TextSentiment/repository/EDS231_TextSentimentAnalysis/assignments/HW02_SentimentAnalysisI/data"
2
files <- list.files(pattern = ".docx", path = data, 
                    full.names = TRUE, recursive = TRUE, ignore.case = TRUE)

dat_invspe <- lnt_read(files)

```

# 2. Choose a key search term or terms to define a set of articles.

The search term I chose is "invasive species".

```{r}
meta_invspe_df <- dat_invspe@meta %>% # 1st tibble in dat
  distinct(Headline, .keep_all = TRUE) 

articles_invspe_df <- dat_invspe@articles # 2nd tibble in dat
paragraphs_invspe_df <- dat_invspe@paragraphs # 3rd tibble in dat

num_invspe_articles <- nrow(meta_invspe_df)
```
